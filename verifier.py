...
# AB SIRF CONFIG IMPORT HOGA
from config import CHECK_INTERVAL, TELEGRAM_BOT_TOKEN, PaymentDB, ConfigDB
...
def parse_famapp_email(html_body):
            ...
            for e_id in data[0].split()[-5:]:
                ...
                if msg.is_multipart():
                    for part in msg.walk():
                        if part.get_content_type() == "text/html": body = part.get_payload(decode=True).decode()
                
                payment_data = parse_famapp_email(body)
